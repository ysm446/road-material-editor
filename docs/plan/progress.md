...
R3 は白線（中央線と外側線の帯ポリゴン）を実装した。R2 の縦断曲線と、道路線形のバンク角（自動 / 手動、回転ギズモ）を実装した。
破線と路肩、デカール、付属物（R4〜R5）は未着手。保存形式は版11。旧地形ノードは移行中の旧実装として残っている。

## 次にやること

- R3 の残り。路肩ノードを Road の Left / Right へ接続して外側境界を次の部品へ渡す。白線の破線・停止線と、Lane Marking の材質接続の手動確認。
