...
道路材質のレイヤー構造（材質スロット × 4 ＋ Road Mask、混ぜ方、下地のハイトで絞る）、テセレーション・変位、車線数と破線・停止線、路肩（Shoulder）、Merge、Decal、ひび割れ（Crack）を実装した。付属物（R5）は未着手。保存形式は版21。旧地形の仕組み（地形ノード、侵食・マスク・ペイントの合成パス、平面プレビュー）は 2026-09-07 に撤去した。
