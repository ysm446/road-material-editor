...
道路材質のレイヤー構造（材質スロット × 4 ＋ Road Mask、混ぜ方、下地のハイトで絞る）、テセレーション・変位、車線数と破線・停止線、路肩（Shoulder）、Merge、Decal、ひび割れ（Crack）を実装した。付属物（R5）は未着手。保存形式は版17。旧地形の仕組み（地形ノード、侵食・マスク・ペイントの合成パス、平面プレビュー）は 2026-09-07 に撤去した。

## 次にやること

- 最優先: [道路・沿道プリセット・区間配置・自動接続への移行計画](roadside-presets.md) の P1。P0は水平直線の4層＋4層＋歩道、GPU計測、サンプル点での変位後誤差検証まで完了。型と保存・Undoの第一段を実装済み。次は完全な材質構成・公開値の結線・配置記述からの評価を統合する。
