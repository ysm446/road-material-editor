...
版27でモデル（`models[]`）、版28で Model ノード（`model`）を追加した。`.tgscene` では `.tgmodel` へ分ける（下記「版27: モデル」）。
道路専用の拡張子は後続で設計する。
...
グラフに `model` ノードを追加した。ピンは出力 Mesh 1 本。設定はノードの `model` 節 `{model, position: [x, y, z], rotation, scale}`。
`model` は `models[].id`（文書内の番号）か `null`（なし）、`position` はモデルの底面の中心の位置（m）、`rotation` は Y 軸まわりの角度（度）、
`scale` はモデルアセットの倍率に掛ける倍率（既定 1、0 以下は 1 として読む）。
版27以前のアプリが読み飛ばして Mesh Output / Merge との接続を失うことを防ぐため版を上げる。

## 版24: 白線の線幅

`roadMarking` に `centerLineWidth`（中央線）、`edgeLineWidth`（左右の外側線）、`laneLineWidth`（同方向の車線境界線）を保存する。単位はm、既定値は各0.15、生成する線の範囲は0.05〜1。中央線は実線・破線とも同じ専用幅を使う。停止線は従来の `stopLineWidth` を維持する。

旧共通値の `lineWidth` は読込互換用に残し、専用キーが無い項目の値として使う。両方無ければ0.15。版24の保存では共通キーを書かない。版23以前のアプリが専用幅を読み飛ばして異なる見た目を表示しないよう、プロジェクトの版を上げる。

## 版17〜18: surfaceLayouts

ルートの `surfaceLayouts` は `{version: 2, nextId, presets: [], layouts: []}`（版17では節の版1）。空の場合も保存する。
...
旧ノードの設定は `src/graph/NodeGraph.h` と `src/io/ProjectIo.cpp` を参照する。
この文書は既存ファイルの読込・移行に必要な仕様として残す。
...
- `road` / `shoulder` の `layerBlendMode`（int ×4。0 マスクどおり / 1 ハイトで競合）— 新規ノードは 0。**キーが無い旧ファイルは 1**（以前の見た目のまま）。[0] は未使用。
...
- `shoulder.displacement`（既定 0）、`shoulder.layerWorldUv`（既定 すべて true）、`shoulder.layerUvRepeat`（既定 1）、`shoulder.layerBlendRange`（既定 0.2）— 路肩の材質レイヤー。Road と同じ意味。
  shoulder の inputs は Path、Material、Material 2〜4、Mask 2〜4 の順（版14〜15 で 2 本だった分は読込時に足す）。
...
- 画像が見つからない → **警告を出して読み込みを続ける。**
  名前・パス・割り当てをリンク切れとして残す。保存しても参照を失わない。
  `.tgmat` も同じ扱いとし、復旧まではマップ未指定相当で評価する。
  テクスチャ一覧からファイル指定・元の場所からの再読み込み・フォルダ検索で繋ぎ直せる。
...
プリセット材質の `enabled` を追加。省略時はtrueで、falseの場合だけ保存する。ノード形式の素材設定にも適用する。元の素材・マスク設定は保持し、評価時だけ下地を定数へ、上層をマスクなしへ置換する。旧版17〜19は従来どおり読み込む。プレビュー専用のカメラ・範囲・表示モードは編集データへ保存しない。

版20では最下層にも移動前のマスク・合成条件を保存できる。評価時だけそれらを無視して全面の下地とし、再び上層へ動かすと元の条件を使う。
