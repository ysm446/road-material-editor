...
uint64_t HashMaskOpParams(uint64_t seed, const MaskOp& op) {
    switch (op.kind) {
        ...
        case MaskOpKind::Area: {
            ...
            hash = HashBytes(hash, &op.pathWorldSpace, sizeof(op.pathWorldSpace));
...
bool MaterialEvaluator::ApplyPathMask(rhi::Device& device, rhi::PipelineCache& pipelineCache,
    ...
    std::vector<PathSegment> normalized;
    if (op.pathWorldSpace) {
        normalized = op.pathSegments;
        const float size = (stack.SizeMeters() > 0.0f) ? stack.SizeMeters() : 1.0f;
        for (PathSegment& segment : normalized) {
            segment.ax = segment.ax / size + 0.5f;
            segment.ay = segment.ay / size + 0.5f;
            segment.bx = segment.bx / size + 0.5f;
            segment.by = segment.by / size + 0.5f;
        }
    }
    const uint32_t segmentsSrv = UploadPathSegments(
        device, index, op.pathWorldSpace ? normalized : op.pathSegments);
